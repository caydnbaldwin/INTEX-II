    ...
    ILogger<WeeklyEmailHostedService> logger) : BackgroundService
{
    ...
    protected virtual Task DelayUntilNextRunAsync(CancellationToken stoppingToken)
        => Task.Delay(TimeSpan.FromHours(1), stoppingToken);

    protected virtual async Task RunAutomationCycleAsync(CancellationToken stoppingToken)
    {
        try
        {
